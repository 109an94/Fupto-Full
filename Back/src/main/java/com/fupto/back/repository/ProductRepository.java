...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p FROM Product p WHERE " +
            ...
            "    (:dateType = 'create_date' AND p.createDate >= :startDate) OR " +
            "    (:dateType = 'update_date' AND p.updateDate >= :startDate)) " +
            "AND (:endDate IS NULL OR :dateType IS NULL OR " +
            "    (:dateType = 'create_date' AND p.createDate <= :endDate) OR " +
            "    (:dateType = 'update_date' AND p.updateDate <= :endDate))")
