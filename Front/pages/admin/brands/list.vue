...
//       `http://localhost:8080/api/v1/admin/brands?page=${currentPage.value}&size=${pageSize.value}&active=${formData.value.active}&nameType=${formData.value.nameType}&name=${formData.value.name}&dateType=${formData.value.dateType}&startDate=${formData.value.startDate}&endDate=${formData.value.endDate}`
//     );
//     const data = await response.json();
//     console.log(data);
//     brands.value = data.brands;
  ...
  <main>
    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex">
          <div>
            <button class="btn btn-outline-danger">전체 삭제</button>
          ...
          <div>
            <router-link to="/admin/brand/reg" class="btn btn-primary">+ Add Brand</router-link>
          </div>
        </div>
        <div>
